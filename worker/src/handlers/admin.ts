...
<div style="display:flex;gap:10px;flex-wrap:wrap">
  ...
  ${(lead.pdfStatus === 'failed' || lead.pdfStatus === 'ready' || lead.pdfStatus === 'pending') ? `<form class="inline" method="POST" action="/admin/regenerate?ref=${encodeURIComponent(ref)}" onsubmit="return confirm('Regenerate PDF and re-send email?')"><button class="btn secondary" type="submit">Regenerate PDF</button></form>` : ''}
